...
export function mount(el, { game: id }, query) {
  ...
  function showIntro() {
    ...
    fill(
      overlay,
      h('h2', first),
      h('p', { class: 'muted' }, names.length > 1 && turn === 0 ? `${names.length} players take turns, one game each, with the same ${id === 'minesweeper' ? 'minefield' : id === 'tetris' ? 'pieces' : 'start'}.` : mod.info.intro),
